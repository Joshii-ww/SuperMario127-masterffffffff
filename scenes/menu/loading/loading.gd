...
func go_to_menu():
	...
	if not Singleton.loaded:
		Singleton.loaded = true
		var number_of_tiles = load("res://assets/tiles/tiles.tres").get_last_unused_tile_id()
		if not (number_of_tiles == Singleton.EditorSavedSettings.data_tiles and ResourceLoader.exists("user://tiles.res", "TileSet")):
